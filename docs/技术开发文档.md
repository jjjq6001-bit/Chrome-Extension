# HBSY VideoGrabber Pro 技术开发文档

> 英文名称：HBSY VideoGrabber Pro  
> 中文名称：HBSY智能视频下载

## 1. 项目概述

### 1.1 项目背景
基于对 Chrome 商店前 5 款视频下载扩展的测试分析，发现以下共同痛点：
- 缺少格式/清晰度选择功能
- 不支持断点续传
- 界面设计陈旧
- 部分需要注册或收集用户数据

### 1.2 产品定位
打造一款**免费、美观、功能完善**的视频下载扩展，解决现有产品的核心痛点。

### 1.3 命名理由
**HBSY VideoGrabber Pro** (HBSY智能视频下载)
- "HBSY" - HBSY品牌标识
- "VideoGrabber" - 视频抓取器，简洁有力
- "Pro" - 体现专业性

---

## 2. 功能需求

### 2.1 核心功能

| 功能 | 优先级 | 描述 |
|------|--------|------|
| 视频检测 | P0 | 自动检测页面中的 video 标签和媒体流 |
| 清晰度选择 | P0 | 支持多种分辨率选择 |
| 格式选择 | P0 | 支持 MP4/WebM/MP3 导出 |
| 断点续传 | P1 | 使用 Range 请求实现分片下载 |
| 多线程下载 | P1 | 并行下载提升速度 |
| 批量下载 | P2 | 一键下载页面所有视频 |
| 下载历史 | P2 | 记录下载历史便于管理 |

### 2.2 界面需求
- 现代化扁平设计
- 深色/浅色主题切换
- 下载进度可视化
- 响应式弹窗布局

---

## 3. 技术架构

### 3.1 整体架构

```
VideoGrabber/
├── manifest.json          # 扩展配置文件 (Manifest V3)
├── src/
│   ├── background/        # Service Worker
│   │   └── index.ts       # 后台脚本，处理下载逻辑
│   ├── content/           # Content Script
│   │   └── detector.ts    # 视频检测脚本
│   ├── popup/             # 弹窗界面
│   │   ├── App.tsx        # React 主组件
│   │   ├── components/    # UI 组件
│   │   └── index.html     # 弹窗入口
│   ├── utils/             # 工具函数
│   │   ├── downloader.ts  # 下载核心逻辑
│   │   ├── parser.ts      # 视频解析器
│   │   └── storage.ts     # 存储管理
│   └── types/             # TypeScript 类型定义
├── public/
│   └── icons/             # 扩展图标
├── package.json
├── tsconfig.json
├── tailwind.config.js
└── vite.config.ts         # 构建配置
```

### 3.2 技术选型

| 技术 | 选择 | 理由 |
|------|------|------|
| 扩展规范 | Manifest V3 | Chrome 最新标准，安全性更高 |
| 语言 | TypeScript | 类型安全，提升代码质量 |
| UI 框架 | React 18 | 组件化开发，生态丰富 |
| 样式 | Tailwind CSS | 快速开发，体积小 |
| 构建工具 | Vite | 快速构建，HMR 支持 |
| 状态管理 | Zustand | 轻量级，适合扩展开发 |

---

## 4. 核心模块设计

### 4.1 视频检测模块 (Content Script)

```typescript
// content/detector.ts
interface VideoInfo {
  id: string;
  url: string;
  title: string;
  duration: number;
  qualities: Quality[];
  thumbnail?: string;
  type: 'video' | 'blob' | 'hls' | 'm3u8';
}

interface Quality {
  label: string;      // "1080p", "720p"
  resolution: string; // "1920x1080"
  bitrate?: number;
  url: string;
}

class VideoDetector {
  private videos: Map<string, VideoInfo> = new Map();
  
  // 检测 <video> 标签
  detectVideoElements(): void {
    const videoElements = document.querySelectorAll('video');
    videoElements.forEach(video => {
      if (video.src) {
        this.addVideo(video.src, 'video');
      }
      // 检测 <source> 子元素
      video.querySelectorAll('source').forEach(source => {
        if (source.src) {
          this.addVideo(source.src, 'video');
        }
      });
    });
  }
  
  // 拦截网络请求检测媒体流
  interceptMediaRequests(): void {
    // 使用 PerformanceObserver 监听资源加载
    const observer = new PerformanceObserver((list) => {
      list.getEntries().forEach((entry) => {
        if (this.isMediaUrl(entry.name)) {
          this.addVideo(entry.name, this.detectType(entry.name));
        }
      });
    });
    observer.observe({ entryTypes: ['resource'] });
  }
  
  private isMediaUrl(url: string): boolean {
    const mediaExtensions = ['.mp4', '.webm', '.m3u8', '.mpd'];
    return mediaExtensions.some(ext => url.includes(ext));
  }
}
```

### 4.2 下载核心模块 (Background Service Worker)

```typescript
// background/downloader.ts
interface DownloadTask {
  id: string;
  url: string;
  filename: string;
  totalSize: number;
  downloadedSize: number;
  status: 'pending' | 'downloading' | 'paused' | 'completed' | 'error';
  chunks: Chunk[];
  speed: number;
  progress: number;
}

interface Chunk {
  start: number;
  end: number;
  downloaded: number;
  status: 'pending' | 'downloading' | 'completed';
}

class DownloadManager {
  private tasks: Map<string, DownloadTask> = new Map();
  private maxConcurrent = 3;  // 最大并发数
  private chunkSize = 1024 * 1024 * 2; // 2MB 分片
  
  // 创建下载任务
  async createTask(video: VideoInfo, quality: Quality): Promise<string> {
    const taskId = crypto.randomUUID();
    const totalSize = await this.getFileSize(quality.url);
    
    const task: DownloadTask = {
      id: taskId,
      url: quality.url,
      filename: `${video.title}_${quality.label}.mp4`,
      totalSize,
      downloadedSize: 0,
      status: 'pending',
      chunks: this.createChunks(totalSize),
      speed: 0,
      progress: 0
    };
    
    this.tasks.set(taskId, task);
    await this.saveTask(task);
    return taskId;
  }
  
  // 分片下载实现断点续传
  private createChunks(totalSize: number): Chunk[] {
    const chunks: Chunk[] = [];
    let start = 0;
    
    while (start < totalSize) {
      const end = Math.min(start + this.chunkSize - 1, totalSize - 1);
      chunks.push({ start, end, downloaded: 0, status: 'pending' });
      start = end + 1;
    }
    
    return chunks;
  }
  
  // 多线程下载
  async startDownload(taskId: string): Promise<void> {
    const task = this.tasks.get(taskId);
    if (!task) return;
    
    task.status = 'downloading';
    const pendingChunks = task.chunks.filter(c => c.status !== 'completed');
    
    // 并行下载多个分片
    const downloadPromises = pendingChunks
      .slice(0, this.maxConcurrent)
      .map(chunk => this.downloadChunk(task, chunk));
    
    await Promise.all(downloadPromises);
  }
  
  // 下载单个分片 (支持 Range 请求)
  private async downloadChunk(task: DownloadTask, chunk: Chunk): Promise<void> {
    const response = await fetch(task.url, {
      headers: {
        'Range': `bytes=${chunk.start + chunk.downloaded}-${chunk.end}`
      }
    });
    
    const reader = response.body?.getReader();
    if (!reader) return;
    
    while (true) {
      const { done, value } = await reader.read();
      if (done) break;
      
      chunk.downloaded += value.length;
      task.downloadedSize += value.length;
      task.progress = (task.downloadedSize / task.totalSize) * 100;
      
      // 保存分片数据到 IndexedDB
      await this.saveChunkData(task.id, chunk, value);
      
      // 通知 UI 更新进度
      this.notifyProgress(task);
    }
    
    chunk.status = 'completed';
  }
  
  // 暂停下载
  pauseDownload(taskId: string): void {
    const task = this.tasks.get(taskId);
    if (task) {
      task.status = 'paused';
      this.saveTask(task);
    }
  }
  
  // 恢复下载 (断点续传)
  async resumeDownload(taskId: string): Promise<void> {
    const task = await this.loadTask(taskId);
    if (task) {
      this.tasks.set(taskId, task);
      await this.startDownload(taskId);
    }
  }
}
```

### 4.3 存储模块

```typescript
// utils/storage.ts
class StorageManager {
  private db: IDBDatabase | null = null;
  
  async init(): Promise<void> {
    return new Promise((resolve, reject) => {
      const request = indexedDB.open('VideoGrabberDB', 1);
      
      request.onupgradeneeded = (event) => {
        const db = (event.target as IDBOpenDBRequest).result;
        
        // 下载任务存储
        if (!db.objectStoreNames.contains('tasks')) {
          db.createObjectStore('tasks', { keyPath: 'id' });
        }
        
        // 分片数据存储
        if (!db.objectStoreNames.contains('chunks')) {
          const store = db.createObjectStore('chunks', { keyPath: ['taskId', 'index'] });
          store.createIndex('taskId', 'taskId');
        }
        
        // 下载历史
        if (!db.objectStoreNames.contains('history')) {
          const store = db.createObjectStore('history', { keyPath: 'id' });
          store.createIndex('date', 'completedAt');
        }
      };
      
      request.onsuccess = () => {
        this.db = request.result;
        resolve();
      };
      
      request.onerror = () => reject(request.error);
    });
  }
  
  // 保存下载任务状态 (用于断点续传)
  async saveTask(task: DownloadTask): Promise<void> {
    const tx = this.db!.transaction('tasks', 'readwrite');
    await tx.objectStore('tasks').put(task);
  }
  
  // 恢复下载任务
  async loadTask(taskId: string): Promise<DownloadTask | null> {
    const tx = this.db!.transaction('tasks', 'readonly');
    return tx.objectStore('tasks').get(taskId);
  }
}
```

---

## 5. UI 设计

### 5.1 弹窗界面组件结构

```typescript
// popup/App.tsx
const App: React.FC = () => {
  return (
    <div className="w-[400px] min-h-[300px] bg-gray-900 text-white">
      <Header />
      <VideoList />
      <DownloadQueue />
      <Footer />
    </div>
  );
};

// 视频列表组件
const VideoList: React.FC = () => {
  const videos = useVideoStore(state => state.videos);
  
  return (
    <div className="p-4 space-y-3">
      {videos.length === 0 ? (
        <EmptyState message="未检测到视频" />
      ) : (
        videos.map(video => (
          <VideoCard key={video.id} video={video} />
        ))
      )}
    </div>
  );
};

// 视频卡片组件
const VideoCard: React.FC<{ video: VideoInfo }> = ({ video }) => {
  const [selectedQuality, setSelectedQuality] = useState(video.qualities[0]);
  
  return (
    <div className="bg-gray-800 rounded-lg p-3 hover:bg-gray-750 transition">
      <div className="flex gap-3">
        {video.thumbnail && (
          <img src={video.thumbnail} className="w-24 h-16 rounded object-cover" />
        )}
        <div className="flex-1">
          <h3 className="font-medium text-sm truncate">{video.title}</h3>
          <p className="text-xs text-gray-400">{formatDuration(video.duration)}</p>
          
          {/* 清晰度选择 */}
          <QualitySelector 
            qualities={video.qualities}
            selected={selectedQuality}
            onChange={setSelectedQuality}
          />
        </div>
        
        {/* 下载按钮 */}
        <DownloadButton video={video} quality={selectedQuality} />
      </div>
    </div>
  );
};
```

### 5.2 设计规范

```css
/* 颜色系统 */
:root {
  --primary: #6366f1;      /* 主色调 - 靛蓝 */
  --primary-hover: #4f46e5;
  --success: #22c55e;      /* 成功 - 绿色 */
  --warning: #f59e0b;      /* 警告 - 橙色 */
  --error: #ef4444;        /* 错误 - 红色 */
  --bg-dark: #111827;      /* 深色背景 */
  --bg-card: #1f2937;      /* 卡片背景 */
  --text-primary: #f9fafb;
  --text-secondary: #9ca3af;
}

/* 圆角规范 */
--radius-sm: 4px;
--radius-md: 8px;
--radius-lg: 12px;

/* 间距规范 */
--spacing-xs: 4px;
--spacing-sm: 8px;
--spacing-md: 16px;
--spacing-lg: 24px;
```

---

## 6. Manifest 配置

```json
{
  "manifest_version": 3,
  "name": "VideoGrabber Pro",
  "version": "1.0.0",
  "description": "智能视频下载扩展 - 支持多清晰度、断点续传、多线程加速",
  "icons": {
    "16": "icons/icon16.png",
    "48": "icons/icon48.png",
    "128": "icons/icon128.png"
  },
  "action": {
    "default_popup": "popup/index.html",
    "default_icon": {
      "16": "icons/icon16.png",
      "48": "icons/icon48.png"
    }
  },
  "background": {
    "service_worker": "background/index.js",
    "type": "module"
  },
  "content_scripts": [
    {
      "matches": ["<all_urls>"],
      "js": ["content/detector.js"],
      "run_at": "document_idle"
    }
  ],
  "permissions": [
    "storage",
    "downloads",
    "activeTab",
    "webRequest"
  ],
  "host_permissions": [
    "<all_urls>"
  ]
}
```

---

## 7. 开发计划

### 7.1 里程碑

| 阶段 | 时间 | 目标 | 状态 |
|------|------|------|------|
| M1 | 第1-2周 | 基础框架搭建，视频检测功能 | ✅ 已完成 |
| M2 | 第3-4周 | 下载核心功能，清晰度选择 | ✅ 已完成 |
| M3 | 第5-6周 | 断点续传，多线程下载 | 🔄 进行中 |
| M4 | 第7-8周 | UI 优化，测试发布 | ⏳ 待开始 |

### 7.2 当前进度 (2024-12-11 更新)

**已完成功能：**
- ✅ 项目基础架构 (Vite + React + TypeScript + Tailwind)
- ✅ Manifest V3 配置
- ✅ 视频检测器 (Content Script)
- ✅ 下载管理器 (Background Service Worker)
- ✅ Popup UI 界面 (视频列表、下载列表、进度显示)
- ✅ 清晰度选择功能
- ✅ 状态管理 (Zustand)
- ✅ 存储模块 (IndexedDB)
- ✅ 工具函数 (格式化、解析)
- ✅ 扩展图标生成

**待完成功能：**
- ⏳ 断点续传完善
- ⏳ 多线程下载优化
- ⏳ HLS/M3U8 流解析
- ⏳ 批量下载
- ⏳ 下载历史记录
- ⏳ 设置面板
- ⏳ 深色/浅色主题切换

### 7.2 技术风险

| 风险 | 影响 | 应对方案 |
|------|------|----------|
| 跨域限制 | 部分视频无法下载 | 使用 background fetch |
| HLS/DASH 流 | 需要特殊处理 | 集成 hls.js 解析 |
| 大文件存储 | IndexedDB 限制 | 分片存储 + 及时清理 |

---

## 8. 竞品对比

| 功能 | HBSY VideoGrabber Pro | Video Downloader Professional | Video DownloadHelper |
|------|------------------|------------------------------|---------------------|
| 清晰度选择 | ✅ | ❌ | ❌ |
| 格式选择 | ✅ | ❌ | ⚠️ 有限 |
| 断点续传 | ✅ | ❌ | ❌ |
| 多线程下载 | ✅ | ❌ | ❌ |
| 现代化 UI | ✅ | ❌ | ❌ |
| 无需注册 | ✅ | ✅ | ✅ |
| 隐私保护 | ✅ | ⚠️ | ⚠️ |
